...
module.exports.getUserID = async function (req, res) {
  try {
    const user = await User.findOne({ cpf: req.params.cpf }); //req.params.body -> fetches using whats on the url
    if(!user){
    ...
    //  console.log(req.params.password);
     if(user){
       bcrypt.compare(req.params.password, user.password, (err,same)=> {
         ...
         if(same == true){
           //console.log("lesgo");
           return res.status(200).json({ success: true, message: "Password matches"});
         }
         //console.log("Doesnt match weeeeeb");
